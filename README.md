# 🌌 Hermes Ultimate Skills Catalog

<div align="center">
  <img src="https://raw.githubusercontent.com/NousResearch/hermes-agent/main/assets/hermes-logo.png" alt="Hermes Logo" width="120" style="border-radius: 24px; margin-bottom: 20px; box-shadow: 0 20px 40px rgba(0,0,0,0.3);"/>
  
  <p align="center">
    <strong>הארכיטקטורה השמימית של סוכן הליבה – 116 סקילים מותקנים וטעונים לפעולה מיידית.</strong>
  </p>

  <p align="center">
    <a href="https://github.com/yurinzon/hermes-skills-catalog"><img src="https://img.shields.io/badge/Hermes_OS-Fully_Loaded-6366f1?style=for-the-badge&logo=gitbook&logoColor=white" alt="Platform"/></a>
    <a href="https://github.com/yurinzon/hermes-skills-catalog"><img src="https://img.shields.io/badge/Total_Skills-116_Active-8b5cf6?style=for-the-badge" alt="Total Skills"/></a>
    <a href="https://github.com/yurinzon/hermes-skills-catalog"><img src="https://img.shields.io/badge/License-MIT-06b6d4?style=for-the-badge" alt="License"/></a>
  </p>
</div>

---

## 🔮 פלטפורמת העל של Hermes (2026 Edition)
ברוכים הבאים לגרסה המתקדמת ביותר של קטלוג הסקילים. דף זה מציג את **כל 116 הסקילים** הזמינים, המותקנים והמותאמים אישית במחשב ה-Mac שלך. כל סקיל כאן מעניק ל-Hermes יכולות-על – החל ממיטוב קוד מורכב, דרך מנועי עיצוב מותאמים אישית ועד לשילוב הדוק עם מערכות macOS ואובסידיאן.

---
## 🤖 מהו Hermes Agent? (The Core Platform)

**Hermes Agent** הוא פריים-וורק סוכני AI בקוד פתוח מבית **Nous Research** המיועד לעבודה ישירות מתוך הטרמינל, פלטפורמות הודעות (כמו Telegram ו-Discord) וסביבות פיתוח (IDEs). הוא נמצא באותה קטגוריית-על של סוכנים אוטונומיים מובילים כמו Claude Code (של Anthropic) ו-Codex (של OpenAI), ומאפשר ביצוע משימות קוד, ניהול מערכות, מחקר ואוטומציות מלאות באמצעות גישה ישירה לכלים במחשב שלך.

### 🌟 למה Hermes שונה מכל סוכן אחר?
1. **שיפור עצמי באמצעות סקילים (Self-Improving Skills) 🎓:** 
   בכל פעם ש-Hermes פותר בעיה מורכבת או מגלה שיטת עבודה מוצלחת, הוא שומר אותה כ**סקיל (Skill)** בפורמט Markdown מקומי. הסקילים הללו נטענים מחדש בשיחות הבאות, ובכך הוא הופך לחכם ומותאם יותר ויותר לסביבה שלך לאורך זמן.
2. **זיכרון מתמשך (Persistent Memory) 🧠:** 
   המערכת שומרת זיכרון רב-סשנים אמיתי לגבי מי אתה, העדפות העיצוב והפיתוח שלך, הגדרות המערכת והיסטוריית העבודה. הכל נשמר בשליטה מלאה שלך (כולל אינטגרציה מלאה ל-Obsidian Vault).
3. **תקשורת רב-ערוצית (Multi-Platform Gateway) 📡:** 
   אותו סוכן בדיוק מורץ ברקע ומחובר לחשבונות ה-Telegram, Discord או ה-Slack שלך, ומאפשר לך לשלוט במחשב, להריץ קוד ולייצר אתרים גם כשאתה רחוק מהמקלדת.
4. **עבודה ללא תלות במודל יחיד (Provider-Agnostic) 🔌:** 
   תוכל להחליף מנועי שפה (LLMs) בשנייה אחת – המערכת תומכת ב-20+ ספקי מודלים (OpenRouter, Anthropic, Google, DeepSeek, OpenAI וכו') עם רוטציה אוטומטית של מפתחות API.

---

## 💾 הוראות התקנה מהירה של Hermes Agent (Installation)

כדי להתקין את המערכת המלאה על גבי מחשב macOS או לינוקס, פתח את הטרמינל והרץ את פקודת ההתקנה הרשמית:

```bash
#  התקנה בלחיצת כפתור אחת (One-Step Installer)
curl -fsSL https://raw.githubusercontent.com/NousResearch/hermes-agent/main/scripts/install.sh | bash
```

---

## ⚙️ הגדרה ראשונית והרצה (Initial Setup & Run)

לאחר סיום ההתקנה, תוכל להגדיר ולהתחיל לעבוד עם הסוכן באמצעות הפקודות הבאות:

```bash
# 🚀 כניסה לצ'אט אינטראקטיבי מיידי בטרמינל
hermes

# 🛠️ הרצת אשף ההגדרות האינטראקטיבי (חיבור פלטפורמות, הגדרות כלים וכו')
hermes setup

# 🔑 בחירה והגדרה מהירה של מודל העבודה הראשי והספק (Provider)
hermes model

# 🩺 בדיקת תקינות מערכת (מוודא תלויות, הרשאות ומפתחות API)
hermes doctor
```

---

## 💬 פקודות סלאש מנצחות (Essential Slash Commands)

במהלך שיחה חיה עם Hermes (בצ'אט או בטרמינל), תוכל לשלוט על התנהגות הסוכן באופן מיידי באמצעות פקודות הסלאש הבאות:

* `/new` או `/reset` – פתיחת סשן נקי ורענן עם ניקוי זיכרון זמני (ללא פגיעה בזיכרון ארוך הטווח).
* `/model [name]` – צפייה או החלפת מודל העבודה תוך כדי שיחה.
* `/skill <name>` – טעינה מיידית של סקיל ספציפי מתוך קטלוג 116 הסקילים המפורט למטה.
* `/voice [on|off]` – הפעלה או כיבוי של מצב קולי (הקראה קולית דינמית של תשובות).
* `/tools` – ניהול והפעלה/כיבוי של כלי מערכת ספציפיים.
* `/help` – הצגת רשימת פקודות השליטה המלאה.

---


## ⚡ התקנה והפעלה מהירה (Quick Installation & Activation)

הסקילים מותקנים מראש במערכת, אך במידה ויש צורך בהתקנה מחדש או בטעינה ידנית:

```bash
# 📥 התקנה מהירה של סקיל מהרשת
hermes skills install <skill-name>

# 🚀 טעינת סקיל ישירות לתוך סשן הטרמינל הפעיל
hermes --skills <skill-name>
```

> **💡 טיפ לעבודה בצ'אט (In-Session Command):** 
> במהלך שיחה עם Hermes, תוכל לטעון כל סקיל באופן מיידי באמצעות פקודת הסלאש: `/skill <skill-name>`
